<template>
    <q-card class="pokemon-card" :id="pokemon.name" :ref="pokemon.name">
        <q-img :ratio="1" :src="pokemon.photo" class="__photo" :class="pokemon.color" decoding="async" />
        <q-card-section class="__content">
            <div class="__title fit row no-wrap justify-start justify-between content-between items-center">
                <h1 class="text-h6 text-weight-bold">{{ this.name }}</h1>
                <h2 class="text-subtitle2 text-weight-bold">
                    #{{ pokemon.id }}
                </h2>
            </div>
